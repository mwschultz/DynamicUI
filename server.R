...
shinyServer(function(input, output, session) {
  ...
  output$sleepPlot <- renderPlot({
    ...
    if (input$conservation) {
    	...
    	if (input$check) {
    		g + geom_point(size = input$size, aes(col = conservation, shape = sleepData$order)) #note that sleep Data is already pulled in, don't need to reference it as a reactive function. 
    	}
    	else {
